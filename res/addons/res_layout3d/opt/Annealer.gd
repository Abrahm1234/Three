...
func run(part: Partition, cost: PlanCost, entry_idx: int, terms: Dictionary) -> void:
	...
	var temperature := _auto_initial_temperature(part, cost, entry_idx, terms)
...
func _auto_initial_temperature(part: Partition, cost: PlanCost, entry_idx: int, terms: Dictionary, samples: int = 32) -> float:
	var rng := RandomNumberGenerator.new()
	rng.randomize()
	...
	for _i in range(samples):
		...
		if not Validate.feasible(part):
			part.restore(backup)
			continue
		var candidate := cost.total(part, entry_idx, terms)
		deltas.append(abs(candidate - base_cost))
		part.restore(backup)
	if deltas.is_empty():
		return max(t0, 0.5)
	deltas.sort()
